...
try:
    ...
    print(Fore.RED + f'[FORTNITEPY] [N/A] [ERROR] Nie Udalo sie zaimportowac 1 lub wiecej modulow, uruchom "PACKAGES.bat".')
    exit()
...
with open('config.json') as f:
    print(f' [FortBOT] [{getTime()}] Loading config.')
    data = json.load(f)
    print(f' [FortBOT] [{getTime()}] Config loaded.')
    
debug = 'False'
if debug == 'True':
    print(f' [FortBOT] [{getTime()}] Debug logging is on.')
    debugOn()
else:
    print(f' [FortBOT] [{getTime()}] Debug logging is off.')
...
async def event_ready():
    print(Fore.GREEN + ' [FortBOT] [' + getTime() + '] Zalogowano jako {0.user.display_name}.'.format(client))
...
async def event_party_invite(invite):
    if data['joinoninvite'].lower() == 'true':
        if invite.sender.display_name not in data['BlockList']:
            try:
                await invite.accept()
                print(Fore.GREEN + f' [FortBOT] [{getTime()}] Zaakceptowano zaproszenie do druzyny od {invite.sender.display_name}')
            except Exception as e:
                pass
        elif invite.sender.display_name in data['BlockList']:
            print(Fore.GREEN + f' [FortBOT] [{getTime()}] Nidgy nie akceptuj do drużyny ' + Fore.RED + f' {invite.sender.display_name}')
    if data['joinoninvite'].lower() == 'false':
        if invite.sender.display_name in data['FullAccess']:
            await invite.accept()
            print(Fore.GREEN + f' [FortBOT] [{getTime()}] Zaakceptowano zaproszenie do druzyny od {invite.sender.display_name}')
        else:
            print(Fore.GREEN + f' [FortBOT] [{getTime()}] Nidgy nie akceptuj do drużyny {invite.sender.display_name}')
            await invite.sender.send(f"Niemoge teraz dolaczyc.")
...
async def event_friend_request(request):
    if data['friendaccept'].lower() == 'true':
        if request.display_name not in data['BlockList']:
            try:
                ...
                print(f" [FortBOT] [{getTime()}] Zaakceptowano zaproszenie do znajomych od {request.display_name}")
            except Exception as e:
                pass
        elif request.display_name in data['BlockList']:
            print(f" [FortBOT] [{getTime()}] Nigdy nie akceptuj zaproszen od " + Fore.RED + f"{request.display_name}")
    if data['friendaccept'].lower() == 'false':
        if request.display_name in data['FullAccess']:
            ...
            print(f" [FortBOT] [{getTime()}] Nigdy nie akceptuj zaproszen od  {request.display_name}")

@client.event
async def event_party_member_join(member):
    if client.user.display_name != member.display_name:
        print(f" [FortBOT] [{getTime()}] {member.display_name} dolaczyl do lobby.")
...
async def event_friend_message(message):
    ...
    print(' [FortBOT] [' + getTime() + '] {0.author.display_name}: {0.content}'.format(message))

    if "!skin" in args[0].lower():
        if message.author.display_name in data['BlockList']:
            await message.reply("Brak Pozwolen!")
        else:
            id = await BenBotAsync.getSkinId(joinedArguments)
            if id == None:
                await message.reply(f"Niemozna znalezc skina: {joinedArguments}")
            else:
                await client.user.party.me.set_outfit(asset=id)
                await message.reply('Ustawiono skina ' + id)
                print(f" [FortBOT] [{getTime()}] Ustawiono skina: " + id)
    ...
    if "!backpack" in args[0].lower():
        if message.author.display_name in data['BlockList']:
            ...
            if len(args) == 1:
                ...
                await message.reply('Plecak Usuniety')
                print(f" [FortBOT] [{getTime()}] Plecak Usuniety")
            else:
                id = await BenBotAsync.getBackpackId(joinedArguments)
                if id == None:
                    await message.reply(f"Niemozna znalezc plecaka: {joinedArguments}")
                else:
                    await client.user.party.me.set_backpack(asset=id)
                    await message.reply('Ustawiono plecak ' + id)
                    print(f" [FortBOT] [{getTime()}] Ustawiono plecak: " + id)
    ...
    if "!emote" in args[0].lower():
        if message.author.display_name in data['BlockList']:
            ...
            if id == None:
                await message.reply(f"Niemozna znalezc emotki: {joinedArguments}")
            else:
                await client.user.party.me.set_emote(asset=id)
                await message.reply('Ustawiono emotke ' + id)
                print(f" [FortBOT] [{getTime()}] Ustawiono emotke: " + id)
    ...
    if "!pickaxe" in args[0].lower():
        if message.author.display_name in data['BlockList']:
            ...
            if id == None:
                await message.reply(f"Niemozna znalezc kilofa: {joinedArguments}")
            else:
                await client.user.party.me.set_pickaxe(asset=id)
                await message.reply('Ustawiono kilof ' + id)
                print(f" [FortBOT] [{getTime()}] Ustawiono kilof: " + id)
        ...
        if message.author.display_name in data['BlockList']:
            ...
            if len(args) == 1:
                ...
                if id == None:
                    ...
                    print(f" [FortBOT] [{getTime()}] Pointing a pickaxe with: " + id)
    ...
    if "!pet" in args[0].lower():
        if message.author.display_name in data['BlockList']:
            ...
            print(f" [FortBOT] [{getTime()}] Zwierzak ustawiony: " + id)
    ...
    if "!emoji" in args[0].lower():
        if message.author.display_name in data['BlockList']:
            ...
            print(f" [FortBOT] [{getTime()}] Client's Emoji set to " + id)

    if "!skullpurple" in args[0].lower():
        if message.author.display_name in data['BlockList']:
            await message.reply("Brak Pozwolen!")
        else:
            try:
                ...
                print(f" [FortBOT] [{getTime()}] Client's Skin set to Purple Skull Trooper")
            except Exception as e:
                pass

    if "!ghoulpink" in args[0].lower():
        if message.author.display_name in data['BlockList']:
            await message.reply("Brak Pozwolen!")
        else:
            try:
                ...
                print(f" [FortBOT] [{getTime()}] Client's Skin set to Pink Ghoul Trooper")
            except Exception as e:
                pass
    ...
    if "!brainiacghoul" in args[0].lower():
        if message.author.display_name in data['BlockList']:
            ...
            try:
                ...
                print(f" [FortBOT] [{getTime()}] Client's Skin set to Brainiac Ghoul Trooper")
            except Exception as e:
                pass
    ...
    if "!purpleportal" in args[0].lower():
        if message.author.display_name in data['BlockList']:
            ...
            print(f" [FortBOT] [{getTime()}] Client's Backpack set to Purple Ghost Portal")
    ...
    if "!banner" in args[0].lower():
        if message.author.display_name in data['BlockList']:
            ...
            print(f" [FortBOT] [{getTime()}] Banner set to; {args[1]} {args[2]} {args[3]}")
    ...
    if "CID_" in args[0]:
        if message.author.display_name in data['BlockList']:
            ...
            print(f' [FortBOT] [{getTime()}] Skin set to ' + args[0])
    ...
    if "!variants" in args[0]:
        if message.author.display_name in data['BlockList']:
            ...
            print(f' [FortBOT] [{getTime()}] Set variants of {args[1]} to {args[2]} {args[3]}.')
    ...
    if "!checkeredrenegade" in args[0].lower():
        if message.author.display_name in data['BlockList']:
            ...
            print(f" [FortBOT] [{getTime()}] Client's Skin set to Checkered Renegade")
    ...
    if "!mintyelf" in args[0].lower():
        if message.author.display_name in data['BlockList']:
            ...
            print(f" [FortBOT] [{getTime()}] Client's Skin set to Minty Elf")
    ...
    if "!echo" in args[0].lower():
        if message.author.display_name in data['FullAccess']:
            ...
            print(f' [FortBOT] [{getTime()}] ' + color.GREEN + 'Sent Message:' + color.END + f' {joinedArguments}')
        else:
            if message.author.display_name not in data['FullAccess']:
                await message.reply(f"Brak Pozwolen!")

    if "!admin" in args[0].lower():
        if message.author.display_name in data['FullAccess']:
            if len(args) == 1:
                await message.reply('Please specify if you want to add or remove a user from the admin list')
                print(f' [FortBOT] [{getTime()}] Please specify if you want to add or remove a user from the admin list, using ' + color.GREEN + '!admin add ' + color.END + 'or ' + color.GREEN + '!admin remove' + color.END)
            if len(args) == 2:
                if args[1].lower() == 'add' or args[1].lower() == 'remove':
                    ...
                    print(f' [FortBOT] [{getTime()}] Please specify the name of the user you want to add/remove from the admin list')
                else:
                    await message.reply('Invalid usage, try !admin add <username> or !admin remove <username>')
                    print(f' [FortBOT] [{getTime()}] Invalid usage, try ' + color.GREEN + '!admin add <username> ' + color.END + 'or ' + color.GREEN + '!admin remove <username>' + color.END)
            if len(args) >= 3:
                    ...
                    if args[1].lower() == 'add':
                        if user.display_name not in data['FullAccess']:
                            ...
                            with open('config.json', 'w') as f:
                                ...
                                print(f" [FortBOT] [{getTime()}] Added " + color.GREEN + f"{user.display_name}" + color.END + " as an admin")
                        elif user.display_name in data['FullAccess']:               
                            print(f" [FortBOT] [{getTime()}]" + color.GREEN + f" {user.display_name}" + color.END + " is already an admin")
                    elif args[1].lower() == 'remove':
                        if user.display_name in data['FullAccess']:
                            data['FullAccess'].remove(user.display_name)
                            with open('config.json', 'w') as f:
                                json.dump(data, f, indent=4)
                                print(f" [FortBOT] [{getTime()}] Removed " + color.GREEN + f"{user.display_name}" + color.END + " as an admin")
                        elif user.display_name not in data['FullAccess']:
                            print(f" [FortBOT] [{getTime()}]" + color.GREEN + f" {user.display_name}" + color.END + " is not an admin")
                except AttributeError:
                    pass
                    print(f" [FortBOT] [{getTime()}] Can't find user: " + color.GREEN + f"{joinedArgumentsAdmin}" + color.END)
        ...
        if message.author.display_name in data['FullAccess']:
            ...
            if len(args) == 2:
                if args[1].lower() == 'add' or args[1].lower() == 'remove':
                    ...
                    print(f' [FortBOT] [{getTime()}] Please specify the name of the user you want to add/remove from the block list')
                else:
                    await message.reply('Invalid usage, try !blocklist add <username> or !blocklist remove <username>')
                    print(f' [FortBOT] [{getTime()}] Invalid usage, try ' + color.GREEN + '!BlockList add <username> ' + color.END + 'or ' + color.GREEN + '!BlockList remove <username>' + color.END)
            if len(args) >= 3:
                joinedArgumentsAdmin = " ".join(args[2:])
                user = await client.fetch_profile(joinedArgumentsAdmin)
                if args[1].lower() == 'add':
                    if user.display_name not in data['FullAccess'] and user.display_name not in data['BlockList']:
                        ...
                        with open('config.json', 'w') as f:
                            ...
                            print(f" [FortBOT] [{getTime()}] Added " + color.GREEN + f"{user.display_name}" + color.END + " to the blocked list.")
                    elif user.display_name in data['FullAccess']:
                        await message.reply(f"{user.display_name} can not be added to the blocked list.")
                        print(f" [FortBOT] [{getTime()}]" + color.GREEN + f" {user.display_name}" + color.END + " cannot be added to the blocked list.")
                    elif user.display_name in data['BlockList']:               
                        await message.reply(f"{user.display_name} is already on the blocked list.")
                        print(f" [FortBOT] [{getTime()}]" + color.GREEN + f" {user.display_name}" + color.END + " is already on the blocked list.")
                elif args[1].lower() == 'remove':
                    if user.display_name in data['BlockList']:
                        data['BlockList'].remove(user.display_name)
                        with open('config.json', 'w') as f:
                            json.dump(data, f, indent=4)
                            print(f" [FortBOT] [{getTime()}] Removed " + color.GREEN + f"{user.display_name}" + color.END + " from the blocked list.")
                    elif user.display_name not in data['BlockList']:
                        print(f" [FortBOT] [{getTime()}]" + color.GREEN + f" {user.display_name}" + color.END + " is not on the blocked list.")
        if message.author.display_name not in data['FullAccess']:
            if len(args) >= 3 and args[1].lower() == 'add':
                await message.reply(f"Password?")
                res = await client.wait_for('friend_message')
    ...
    if "!status" in args[0].lower():
        if message.author.display_name in data['FullAccess']:
            ...
            print(f' [FortBOT] [{getTime()}] Status set to {joinedArguments}.')
        else:
            if message.author.display_name not in data['FullAccess']:
                await message.reply(f"Brak Pozwolen!")
    ...
    if "!leave" in args[0].lower():
        if message.author.display_name in data['FullAccess']:
            ...
            print(Fore.GREEN + f' [FortBOT] [{getTime()}] Left the party as I was requested.')
        else:
            if message.author.display_name not in data['FullAccess']:
                await message.reply(f"Brak Pozwolen!")
    ...
    if "!kick" in args[0].lower() and message.author.display_name in data['FullAccess']:
        ...
        if member is None:
            ...
            try:
                ...
                print(Fore.GREEN + f" [FortBOT] [{getTime()}] Kicked user: {member.display_name}")
            except Exception as e:
                pass
                await message.reply(f"Couldn't kick {member.display_name}, as I'm not party leader.")
                print(Fore.RED + f" [FortBOT] [{getTime()}] [ERROR] Failed to kick member as I don't have the required permissions." + Fore.WHITE)
        if message.author.display_name not in data['FullAccess']:
            await message.reply(f"Brak Pozwolen!")
    ...
    if "!join" in args[0] and message.author.display_name in data['FullAccess']:
        ...
        if friend is None:
            ...
            print(Fore.RED + f" [FortBOT] [{getTime()}] [ERROR] Unable to join user: {joinedArguments}, are you sure the bot has them added?" + Fore.WHITE)
        if message.author.display_name not in data['FullAccess']:
            await message.reply(f"Brak Pozwolen!")
        else:
            try:
    ...
    if "!invite" in args[0].lower():
        ...
        if friend is None:
            await message.reply(f"Unable to invite that user, are you sure the bot has them added?")
            print(Fore.RED + f" [FortBOT] [{getTime()}] [ERROR] Unable to invite user: {joinedArguments}, are you sure the bot has them added?" + Fore.WHITE)
        else:
            try:
                await friend.invite()
                await message.reply(f"Invited user: {friend.display_name}.")
                print(Fore.GREEN + f" [FortBOT] [{getTime()}] Invited user: {friend.display_name}")
            except Exception as e:
                pass
                await message.reply(f"Something went wrong trying to invite {friend.display_name}")
                print(Fore.RED + f" [FortBOT] [{getTime()}] [ERROR] Something went wrong while trying to invite {friend.display_name}" + Fore.WHITE)           
    ...
    if "!add" in args[0].lower() and message.author.display_name in data['FullAccess']:
        ...
        if user is None:
            ...
            print(Fore.RED + f" [FortBOT] [{getTime()}] [ERROR] Unable to find a player with the name {joinedArguments}")
        else:
            try:
                if (user.id in friends):
                    await message.reply(f"I already have {user.display_name} as a friend.")
                    print(Fore.RED + f" [FortBOT] [{getTime()}] [ERROR] You already have {user.display_name} added as a friend.")
                else: 
                    await client.add_friend(user.id)
                    await message.reply(f"Sent a friend request to {user.display_name}")
                    print(Fore.GREEN + f" [FortBOT] [{getTime()}] {client.user.display_name} sent a friend request to {user.display_name}" + Fore.WHITE)
            except Exception as e:
                pass
                print(Fore.RED + f" [FortBOT] [{getTime()}] [ERROR] Something went wrong adding {joinedArguments}" + Fore.WHITE)
        if message.author.display_name not in data['FullAccess']:
            await message.reply(f"Brak Pozwolen!")
        ...
        if user is None:
            ...
            try:
                if (user.id in friends):
                    ...
                    print(Fore.GREEN + f" [FortBOT] [{getTime()}] {client.user.display_name} removed {user.display_name} as a friend.")
                else: 
                    await message.reply(f"I don't have {user.display_name} as a friend.")
                    print(Fore.RED + f" [FortBOT] [{getTime()}] [ERROR] {client.user.display_name} tried removing {user.display_name} as a friend, but the client doesn't have the friend added." + Fore.WHITE)
            except Exception as e:
                pass
                print(Fore.RED + f" [FortBOT] [{getTime()}] [ERROR] Something went wrong removing {joinedArguments} as a friend." + Fore.WHITE)
        if message.author.display_name not in data['FullAccess']:
            await message.reply(f"Brak Pozwolen!")
    ...
    if "!showfriends" in args[0].lower() and message.author.display_name in data['FullAccess']:
        ...
        try:
            ...
            print(f" [FortBOT] [{getTime()}] " + Fore.WHITE + "Friends List: " + Fore.GREEN + f"{len(onlineFriends)} Online " + Fore.WHITE + "/" + Fore.LIGHTBLACK_EX + f" {len(offlineFriends)} Offline " + Fore.WHITE + "/" + Fore.LIGHTWHITE_EX + f" {len(onlineFriends) + len(offlineFriends)} Total")
    ...
    if "!members" in args[0].lower() and message.author.display_name in data['FullAccess']:
            ...
            print(f" [FortBOT] [{getTime()}] " + Fore.WHITE + "There are " + Fore.LIGHTWHITE_EX + f"{len(partyMembers)} members in client's party:")
    ...
    if "!promote" in args[0].lower() and message.author.display_name in data['FullAccess']:
        ...
        if member is None:
            ...
            try:
                ...
                print(Fore.GREEN + f" [FortBOT] [{getTime()}] Promoted user: {member.display_name}")
            except Exception as e:
                pass
                await message.reply(f"Couldn't promote {member.display_name}, as I'm not party leader.")
                print(Fore.RED + f" [FortBOT] [{getTime()}] [ERROR] Failed to promote member as I don't have the required permissions." + Fore.WHITE)
        if message.author.display_name not in data['FullAccess']:
            await message.reply(f"Brak Pozwolen!")
    ...
    if "Playlist_" in args[0]:
        try:
            ...
            print(Fore.RED + f" [FortBOT] [{getTime()}] [ERROR] Failed to set gamemode as I don't have the required permissions." + Fore.WHITE)
    ...
    if args[0] == "!id":
        if message.author.display_name in data['BlockList']:
            ...
            try:
                ...
                print(Fore.GREEN + f" [FortBOT] [{getTime()}] {joinedArguments}'s Epic ID is: {user.id}")
            except AttributeError:
                await message.reply(f"I couldn't find an Epic account with the name: {joinedArguments}.")
                print(Fore.RED + f" [FortBOT] [{getTime()}] [ERROR] I couldn't find an Epic account with the name: {joinedArguments}.")

try:
    client.run()
except fortnitepy.AuthException:
    print(Fore.RED + f" [FortBOT] [{getTime()}] [ERROR] Couldn't log into the account, are the account credentials correct?")
